...
| `test_api.py` | `202` shape, `404`, field mutual exclusion per status, `422` on bad input; that a `POST` really schedules the run and `TestClient` really drains it; `class` as the served key of a `Violation`; the lifespan's repository ownership; and that `/metrics` is served and shares its registry with the background run |
