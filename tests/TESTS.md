...
| `test_tracing.py` | step ordering, `seq` monotonicity, **`ts` increasing with `seq`**, summarisation rules; the optional `on_step` hook fires once per step and is off by default |
| `test_api.py` | `202` shape, `404`, field mutual exclusion per status, `422` on bad input; that a `POST` really schedules the run and `TestClient` really drains it; `class` as the served key of a `Violation`; the lifespan's repository ownership; that the edge and the domain agree at the exact length boundary because they share one definition of it; the two states `/health` may report and their presence in the published schema; and that `/metrics` is served and shares its registry with the background run |
...
| `test_demo.py` | the ten demo scenarios ([DEMO.md](../src/support_assistant/demo/DEMO.md)), each driven through the real pipeline and held to the outcome it claims to demonstrate — the one file that walks all four handoff reasons reachable through the API, and that pins the fixture-to-path map TOOLS.md states in prose; plus that `/ui` is served and that every endpoint its JavaScript calls exists in the published OpenAPI paths |
